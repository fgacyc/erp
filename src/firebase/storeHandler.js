...
export class FBStore {
	...
	readDocument(collectionName, documentID) {
		...
		if (this.validate(documentID) !== 'string')
			throw new Error(
				'Invalid documentID, expected string, got ' + typeof documentID,
			);
	...
	addArrayElement(collectionName, documentID, fieldName, element) {
		...
		if (this.validate(fieldName) !== 'string')
			throw new Error(
				'Invalid fieldName, expected string, got ' + typeof fieldName,
			);
	...
	addNum(collectionName, documentID, fieldName, number) {
		...
		if (typeof number !== 'number')
			throw new Error('Invalid number, expected number, got ' + typeof number);
	...
	validateThreeParams(collectionName, document, documentID) {
		if (arguments.length !== 3)
			throw new Error(
				'Invalid number of arguments, expected 3, got ' + arguments.length,
			);
		...
		if (this.validate(document) !== 'object')
			throw new Error(
				'Invalid document, expected object, got ' + typeof document,
			);
