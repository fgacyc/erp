...
export default function EmailOrPhoneSettingModal({
	visible,
	setVisible,
	type,
}) {
	...
	const [email, updateEmail] = useSettingModalStore(
		(state) => [state.email, state.setEmail],
		shallow,
	);
	const [phoneNumber, updatePhoneNumber] = useSettingModalStore(
		(state) => [state.phone_number, state.setPhoneNumber],
		shallow,
	);
