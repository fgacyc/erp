...
export  function checkApplicantStatus(recruiter){
    ...
    else if(recruiter.interview.status === true && recruiter.application.status === "pre-accepted"){
        return 2.5; // waiting for evaluation
    }

    // interview: appointment, waiting for interview, interview done
    else if (recruiter.application.status === "pre-accepted" && recruiter.interview.status === true) {
        return 2; //interivew done
    }
    else if(recruiter.application.status === "pre-accepted" && recruiter.hasOwnProperty("appointment")){
        return 1.5; // appointment and waiting for interview
    }
    else if(recruiter.application.status === "pre-accepted" && !recruiter.hasOwnProperty("appointment") && recruiter.interview.status !== null){
...
function interviewInfo(statusID,recruiter){
    if(statusID === 1.5){
        ...
        let interviewStartTime = getDateString(recruiter.interview.ministry.start_time * 1000)
        let interviewEndTime = getDateString(recruiter.interview.ministry.end_time * 1000)
